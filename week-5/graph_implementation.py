...
class Digraph(object):
    ...
    # returning each destionation for a given node
    def childrenOf(self, node):
        return self.edges[node]
    # returns boolean for a given node exists in the graph
...
def DFSShortest(graph, start, end, path = [], shortest = None):
    ...
    # adding with + makes it a stack, each new entry is added to the beginning of the list.
    path = path + [start]
    print 'Current dfs path:', printPath(path)
    if start == end:
        return path
...
def BFS(graph, start, end, q=[]):
    initPath = [start]
    # adding with append makes q a queue. This adds each new entry to the beginning of the code.
    q.append(initPath)
    while len(q) != 0:
        tmpPath = q.pop(0)
